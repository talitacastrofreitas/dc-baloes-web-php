...
// Buscar padrões de configurações
$set_res = sqlsrv_query($conn, "SELECT * FROM Settings WHERE id = 1");
$defaults = sqlsrv_fetch_array($set_res, SQLSRV_FETCH_ASSOC);
?>
    ...
    <div class="row">
        <div class="col-md-7">
            <div class="card p-4 border-0 shadow-sm rounded-4">
                ...
                <form action="processar_calculo.php" method="POST" id="formCalculadora" onsubmit="return validarValorMinimo()">
                    ...
                    <input type="text" name="client_phone" class="form-control mb-3" placeholder="Telefone (Ex: 5571999999999)" required>
                    ...
                    <select name="service_type" id="service_type" class="form-select mb-3" required onchange="atualizarMinimo()">
                        ...
                        <?php 
                        $serv_res = sqlsrv_query($conn, "SELECT * FROM Services");
                        while ($s = sqlsrv_fetch_array($serv_res, SQLSRV_FETCH_ASSOC)) {
                            $min_formatado = number_format($s['min_value'], 2, ',', '.');
                            echo "<option value='{$s['service_name']}' data-min='{$s['min_value']}' data-desc='{$s['service_description']}'>{$s['service_name']} (A partir de R$ {$min_formatado})</option>";
                        }
                        ?>
                    </select>
                    ...
                    <div class="row g-3">
                        <div class="col-6"><label class="small">Materiais (R$)</label>
                            <input type="number" step="0.01" name="material_cost" id="mat_cost" class="form-control" required oninput="calcularPrevia()"></div>
                        <div class="col-6"><label class="small">Lucro (%)</label>
                            <input type="number" name="markup" id="markup" class="form-control" value="<?php echo $defaults['default_markup_percentage']; ?>" oninput="calcularPrevia()"></div>
                        <div class="col-6"><label class="small">Horas</label>
                            <input type="number" step="0.1" name="labor_hours" id="hours" class="form-control" oninput="calcularPrevia()"></div>
                        ...
                        <div class="col-12"><label class="small">Taxa Entrega (R$)</label>
                            <input type="number" step="0.01" name="delivery_fee" id="delivery" class="form-control" value="0" oninput="calcularPrevia()"></div>
                    </div>
        ...
        <div class="col-md-5">
            ...
            <div class="list-group">
                <?php 
                $query = sqlsrv_query($conn, "SELECT TOP 10 * FROM PriceCalculation ORDER BY created_date DESC");
                    ...
                    <div class="list-group-item list-group-item-action border-0 shadow-sm rounded-3 mb-2 p-3">
                        <div class="d-flex justify-content-between align-items-center">
                            ...
                            <div class="text-end">
                                ...
                                <div class="mt-2">
                                    <button class="btn btn-sm btn-light p-1" onclick='verDetalhes(<?php echo json_encode($row); ?>)'>👁️</button>
                                    <a href="acoes_calculo.php?del=<?php echo $row['id']; ?>" class="btn btn-sm btn-light p-1 text-danger" onclick="return confirm('Excluir orçamento?')">🗑️</a>
                                </div>
                            </div>
                        </div>
                    </div>
...
function atualizarMinimo() {
    ...
    if(descricaoServico !== '') { document.getElementById('desc_servico').value = descricaoServico; }
